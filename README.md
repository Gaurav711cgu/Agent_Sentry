...
|---|---|---|
| **OS Kernel System Call Filtering** | Linux `prctl(PR_SET_SECCOMP)` BPF filter (`syscall_sandbox.py`) blocking `execve`, `socket`, `open`, `fork` | **0 Unsafe Kernel Syscall Breaches** |
| **Obfuscation & Homoglyph Defense** | Base64/Hex decoding + Unicode `NFKC` homoglyph transliteration map (`bypass_detector.py`) | **99.20% GARAK Exploit Block Rate** |
...
| **Ultra-Low Latency AST Scanning** | Recursive Python AST parser (`ast_parser.py`) inspecting subshell syntax nodes | **13.90 µs Median Scan Latency** (p99 20.88 µs) |
| **Suffix-Delta Prompt Caching** | Dynamic system prompt header reordering pushing user deltas to suffix | **50.56% Token Cost Reduction** (<0.01ms overhead) |
...
flowchart TD
    ...
    L2 --> L3["Layer 3: Recursive AST & OWASP Pattern Scanner (ast_parser.py)"]
    L3 -->|Unsafe Syntax| BLOCK["Block & Log Security Alert"]
    L3 -->|Safe Payload| L4["Layer 4: Linux Seccomp-BPF & POSIX Rlimits (syscall_sandbox.py)"]
...
AgentSentry applies kernel-level BPF (Berkeley Packet Filter) rules directly to the child process via `prctl(PR_SET_SECCOMP)`:
```python

...
class AgentSentryGateway:
    ...
    async def execute_llm_call_with_retry(self, provider: str, payload: Dict[str, Any], max_retries: int = 3) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries + 1):
            try:
                # Mocking network latency & random provider rate limit failure (10% rate)
                ...
                if attempt == max_retries:
                    ...
                    # Failover fallback logic: Anthropic -> OpenAI / OpenAI -> Anthropic
    ...
    async def route_tool_call_async(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        # We can run the CPU-intensive static analysis segment inside a thread pool
        # to ensure no thread blocking occurs on the FastAPI server main threads.
...
def create_app(gateway: AgentSentryGateway) -> FastAPI:
    ...
    @app.post("/v1/tools/execute-safe")
